...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    // BACKEND INTEGRATION: This function would check if the product is in the user's wishlist
    // You would implement a proper API endpoint that connects to your database
    // and checks if the product is in the user's wishlist
  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // In a real app, this would submit to a reviews API endpoint
  ...
  if (!product) {
    return (
      <div className="container mx-auto px-4 py-12 text-center">
        ...
        <p className="text-blue-700 mb-8">The product you are looking for does not exist or has been removed.</p>
    ...
    <div className="container mx-auto px-4 py-12">
      <div className="flex flex-col md:flex-row gap-8">
            ...
            {productImages.map((image, index) => (
              <button
                ...
                className={`relative h-20 w-20 rounded-md overflow-hidden border-2 transition-all ${
                  activeImage === index ? "border-blue-600" : "border-blue-100 hover:border-blue-300"
                }`}
        ...
        <div className="md:w-1/2">
            ...
            <div className="flex">
              {[...Array(5)].map((_, i) => (
                <Star
                  key={i}
                  className={`h-5 w-5 ${
                    i < Math.round(averageRating) ? "fill-blue-500 text-blue-500" : "fill-gray-200 text-gray-200"
                  }`}
                />
              ))}
            </div>
          ...
          <p className="text-2xl font-bold text-blue-800 mb-6">₦{product.price.toLocaleString()}</p>
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center">
              ...
              <span className="ml-4 text-sm text-blue-600">{product.quantity} available</span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-8">
            ...
            >
              <Heart className={`mr-2 h-5 w-5 ${isInWishlist ? "fill-pink-600" : ""}`} />
          ...
          <Card className="border-blue-100">
            <CardContent className="p-4">
              ...
              <p className="text-sm text-blue-600">Free standard shipping on orders over ₦20,000</p>
            </CardContent>
          </Card>
        </div>
      </div>
        ...
        <Tabs defaultValue="description">
          ...
          <TabsContent value="description" className="py-6">
            <div className="prose prose-blue max-w-none">
              ...
              <p>
                Our {product.name} is crafted with attention to detail and made from high-quality materials to ensure
                durability and longevity. The elegant design adds a touch of sophistication to any room, making it a
                perfect addition to your home decor collection.
              </p>
              <p>
                The versatile nature of this piece allows it to complement various interior styles, from modern
                minimalist to classic traditional. Its blue tones create a calming atmosphere, perfect for relaxation
          ...
          <TabsContent value="specifications" className="py-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="border-b border-blue-100 pb-2">
                <span className="font-medium text-blue-900">Care Instructions</span>
                <p className="text-blue-700">Hand wash only</p>
              </div>
              <div className="border-b border-blue-100 pb-2">
                <span className="font-medium text-blue-900">Country of Origin</span>
          ...
          <TabsContent value="reviews" className="py-6">
            {user && (
              <Card className="mb-8 border-blue-100">
                <CardContent className="p-6">
                  <h3 className="text-lg font-medium text-blue-900 mb-4">Write a Review</h3>
                  <form onSubmit={handleReviewSubmit}>
                    ...
                    <div className="mb-4">
                      <label htmlFor="review-title" className="block text-sm font-medium text-blue-700 mb-1">
                        Title
                      </label>
                      <input
                        id="review-title"
                    ...
                    <div className="mb-4">
                      <label htmlFor="review-comment" className="block text-sm font-medium text-blue-700 mb-1">
                        Review
                      </label>
                      <textarea
                        id="review-comment"
            ...
            {reviewsLoading ? (
              <div className="space-y-6">
                {[...Array(3)].map((_, i) => (
                  <div key={i} className="border-b border-blue-100 pb-6 animate-pulse">
              ...
              <div className="text-center py-8">
                <p className="text-blue-700 mb-4">This product doesn't have any reviews yet.</p>
                {!user && (
                  <Button asChild className="bg-blue-600 hover:bg-blue-700">
                    <Link href={`/login?redirect=/products/${id}`}>Login to Write a Review</Link>
                  </Button>
                )}
              </div>
            ) : (
              <div className="space-y-6">
                {reviews.map((review) => (
                  <div key={review.id} className="border-b border-blue-100 pb-6">
                      ...
                      <div className="flex mr-2">
                        {[...Array(5)].map((_, j) => (
                          <Star
                            key={j}
                            className={`h-4 w-4 ${
                              j < review.rating ? "fill-blue-500 text-blue-500" : "fill-gray-200 text-gray-200"
                            }`}
                          />
                        ))}
                      </div>
                      <span className="font-medium text-blue-900">{review.title}</span>
                    </div>
                ...
                {reviews.length > 3 && <Button className="bg-blue-600 hover:bg-blue-700">Load More Reviews</Button>}
              </div>
            )}
          </TabsContent>
        </Tabs>
